...
"classification": {"rules": "ONE_TIME_FIXED", "jev": "RECURRING_VARIABLE", "jev_confidence": 0.86, "probabilities": {}, "agree": false, "contradictions": [], "final": "ONE_TIME_FIXED", "cache_hit": false}
...
1. `evidence.run(ws, llm, jev, period)` - canonicalisation. Copies visible world tables into db, and for each visible unprocessed document: LLM `extract` -> record; ONE Jev call verifies it against the text (Nouls: amount supported, vendor identity consistent, dates supported; Choice: document type incl. OTHER) -> `documents` row with `quality: OK|NEEDS_REVIEW`; applies OK records to db tables `contracts`, `terminations`, `activity`, `goods_receipts`, each stamped with the document's `available_at` (workers read the db through `store.visible`). Hash-cached per document.
2. `detection.run(ws, jev, period)` - obligation discovery from db tables by rules (validity / contract coverage / received > billed / activity in period / card transactions in period). Jev only for ambiguous signals (e.g. contract expired but invoices historically continue; termination text unclear). Creates `DETECTED` cases; keeps CLOSED+ cases untouched.
3. `invoice_lookup.run(ws, jev, period)` - deterministic search (vendor, PO, line, period, amount tolerance, entity) -> one of the six results; Jev Choice among candidates only when several are plausible; Noul for same-obligation duplicates.
4. `classifier.run(ws, jev, period)` - rule tree on structured fields + extra contradiction checks; Jev Choice over the four categories (+OTHER) from description/contract excerpt; compare; cache by sha256(po_line_id + modified_at + contract version). After 3 and 4 the engine sets `ENRICHED` and resolves.
5. `estimation.run(ws, jev, llm, period)` - router: rules pick the estimator when `recognition_basis` + inputs are unambiguous, else Jev Choice. Same Jev call asks Score evidence sufficiency and Noul plausibility. Code computes. Applies ACTIVE rules from `rules.py`. Insufficient -> `OUTREACH_PENDING`.
