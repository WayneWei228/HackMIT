...
from .fakes import FakeJev, make_ws
...
def test_none_queue_and_posted(tmp_path):
    ...
    cases = invoice_lookup.run(ws, FakeJev({}), P)
...
def test_goods_line_partial_then_full(tmp_path):
    ...
    m = match(invoice_lookup.run(ws, FakeJev({}), P), "PO-008-001")
...
def test_duplicate_candidate(tmp_path):
    ...
    (c,) = invoice_lookup.run(ws, FakeJev({}), P)
...
def test_future_invoice_is_invisible(tmp_path):
    ...
    assert match(invoice_lookup.run(ws, FakeJev({}), P), "PO-001-001")["result"] == "NO_INVOICE"
    assert match(invoice_lookup.run(ws.at("2027-01-25T12:00:00Z"), FakeJev({}), P), "PO-001-001")["result"] == "INVOICE_IN_QUEUE"


def test_unreferenced_invoice_single_line_needs_no_jev(tmp_path):
    ws = setup(tmp_path, [header("PO-001")], [line("PO-001")], [inv("I-NOREF", None, 1200)])
    jev = FakeJev({})
    assert match(invoice_lookup.run(ws, jev, P), "PO-001-001")["invoice_ids"] == ["I-NOREF"] and jev.calls == []


def two_lines(tmp_path):
    return setup(tmp_path, [header("PO-013"), header("PO-014")],
...
def test_unreferenced_invoice_jev_chooses(tmp_path):
    ws = two_lines(tmp_path)
    jev = FakeJev({"which_line": {"type": "choice", "choice": "PO-014-001", "confidence": 0.95, "probabilities": {}}})
    cases = invoice_lookup.run(ws, jev, P)
    assert match(cases, "PO-014-001")["result"] == "INVOICE_IN_QUEUE" and match(cases, "PO-013-001")["result"] == "NO_INVOICE"
    tag, state, questions = jev.calls[0]
    assert tag == "invoice_lookup:I-NOREF" and set(state["po_lines"]) == {"PO-013-001", "PO-014-001"}
    assert set(questions["which_line"].criteria) == {"PO-013-001", "PO-014-001", "NONE"}
    assert any(d["kind"] == "JEV" for d in next(c for c in cases if c["case_key"] == "PO-014-001")["decision_log"])
    assert store.load_table(ws, "invoices")[0]["po_line_id"] is None  # the table is Evidence's; lookup does not rewrite it


def test_unreferenced_invoice_low_confidence_is_ambiguous(tmp_path):
    ws = two_lines(tmp_path)
    jev = FakeJev({"which_line": {"type": "choice", "choice": "PO-014-001", "confidence": 0.58, "probabilities": {}}})
    cases = invoice_lookup.run(ws, jev, P)
    for key in ("PO-013-001", "PO-014-001"):
        c = next(c for c in cases if c["case_key"] == key)
        assert c["invoice_match"]["result"] == "AMBIGUOUS_MATCH" and c["flags"] == ["AMBIGUOUS_MATCH"]
        assert c["invoice_match"]["candidates"][0]["invoice_id"] == "I-NOREF"
    again = invoice_lookup.run(ws, jev, P)  # re-run: no duplicated flags or log lines
