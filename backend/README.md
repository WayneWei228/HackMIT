...
Without `ANTHROPIC_API_KEY` the gateway falls back to the local `claude` CLI.

## Layout

| Path | Role |
|---|---|
| `trueup/gateway/` | `llm.py` (Claude), `jev.py` (Jev classifier), `tracing.py` |
...
| `trueup/agents/` | Evidence, detection, invoice lookup, classifier, outreach, learning |
...
- Claude narration of each estimate and Claude diagnosis with quoted evidence (the deterministic diagnosis is a placeholder).
